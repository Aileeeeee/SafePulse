...
function FilterDropdown({ label, options, value, onChange }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 px-4 py-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:border-gray-400 transition-colors shadow-sm"
      >
...
export default function IncidentsPage({ onSelectIncident }) {
  ...
  return (
    <div className="px-8 py-7 flex flex-col gap-6 h-full">
      {/* Heading */}
      <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {/* Header row */}
        <div className="grid grid-cols-[100px_1fr_1fr_1fr_90px_160px] gap-x-4 px-6 py-3 border-b border-gray-100 bg-gray-50/60">
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((inc, i) => (
            <div
              ...
              className={`grid grid-cols-[100px_1fr_1fr_1fr_90px_160px] gap-x-4 px-6 py-4 items-center cursor-pointer transition-colors hover:bg-gray-50
                ${i < filtered.length - 1 ? 'border-b border-gray-100' : ''}`}
            >
              <span className="text-sm font-semibold text-gray-700">#{inc.id}</span>
              <span className="text-sm text-gray-800">{inc.category}</span>
